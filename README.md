...
### [수직 슬라이스 우선 — 수평 확장 이전에](confirmed/execution/vertical-slice-before-horizontal-scale.md)

파이프라인의 어느 스테이지도 전 스테이지를 관통한 샘플 하나가 완전히 통과되기 전까지는 수평으로 확장할 수 없다. 스테이지 k가 하류 스테이지를 거치지 않은 상태에서 스케일되면 실패는 조용히 전파되어 터미널 출력에서만 표면화된다. 실패 귀인이 붕괴된다 — 결함의 발생 위치를 터미널 출력만으로는 결정할 수 없다. 모델 능력과 무관하다.

### [수집은 검증 계약을 앞서지 않는다](confirmed/execution/acquisition-must-not-outrun-validation-contract.md)

수집 단계는 하류 검증 계약(필드 스키마, 증거 기준, 수락 게이트)이 고정·테스트·잠금되기 전에 탐색 프로브 볼륨을 초과해 스케일할 수 없다. 구조적 드라이버는 비용 비대칭과 비가역성이다. 잠긴 계약 없이 대량 샘플이 수집되면 출력은 자산이 아닌 부채가 된다. 완벽한 원샷 모델이 수천 개의 미계약 샘플을 생성해도 귀인 불가능한 대량 출력이라는 결과는 동일하다.

### [스테이지 로컬 완결과 수리](confirmed/execution/stage-local-completion-and-repair.md)

멀티스테이지 파이프라인의 각 스테이지는 스테이지 로컬 DONE 게이트를 소유해야 한다. 새 입력 변종이 파이프라인을 깨뜨리면, 수리는 로컬 게이트가 실패한 가장 이른 스테이지에만 국한된다. 전역 최종 게이트 하나만 있으면 어느 스테이지가 깨졌는지 숨겨지고 모든 실패에서 전체 재실행이 강제된다. 파이프라인 길이에 비례해 수리 비용이 복리로 증가한다.
